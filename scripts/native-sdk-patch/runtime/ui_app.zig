...
pub fn UiAppWithFeatures(comptime ModelT: type, comptime MsgT: type, comptime features: UiAppFeatures) type {
    return struct {
        ...
        pub const Options = struct {
            ...
            /// Optional mapping from open-document events (macOS: the
            /// launch-time and later `odoc` AppleEvents Finder, Open
            /// With, and `open -a App file` deliver) into messages so
            /// the model can open the file(s) itself. The `paths` slices
            /// point at the platform event payload and stay valid only
            /// for the duration of the synchronous dispatch — copy
            /// anything the model keeps.
            on_open_files: ?*const fn (paths: []const []const u8) ?MsgT = null,
        ...
        fn eventFn(context: *anyopaque, runtime: *Runtime, event_value: Event) anyerror!void {
            ...
            switch (event_value) {
                ...
                .open_files => |open| if (self.options.on_open_files) |map| {
                    if (map(open.paths)) |msg| {
                        try self.dispatch(runtime, self.canvas_window_id, msg);
                    }
                },
