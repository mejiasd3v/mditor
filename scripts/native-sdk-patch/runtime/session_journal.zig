...
pub fn encodeEvent(event: platform.Event, buffer: []u8) JournalError![]const u8 {
    ...
    switch (event) {
        ...
        .open_files => |open| {
            try cursor.writeEnum(EventTag.open_files);
            try cursor.writeInt(u64, open.window_id);
            if (open.paths.len > max_session_drop_paths) return error.JournalRecordOverBudget;
            try cursor.writeInt(u16, @intCast(open.paths.len));
            for (open.paths) |path| try cursor.writeStr(path);
        },
