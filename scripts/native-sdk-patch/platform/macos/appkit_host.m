...
/* NSApplicationDelegate bridging the open-document AppleEvent (Finder
 * double-click, Open With, `open -a App file`) into an OPEN_FILES
 * event: modern macOS delivers launch documents by AppleEvent only —
 * the argv the process sees is just the binary path — so a file-handling
 * app must answer `application:openFiles:`. The host is referenced
 * weakly; the app delegate object is retained by NativeSdkAppKitHost. */
...
@property(nonatomic, strong) NativeSdkAppDelegate *appDelegate;
...
- (void)emitPaths:(NSArray<NSString *> *)paths {
    ...
    [host emitEvent:(native_sdk_appkit_event_t){
        .kind = NATIVE_SDK_APPKIT_EVENT_OPEN_FILES,
        .window_id = 1,
...
- (instancetype)initWithAppName:(NSString *)appName displayName:(NSString *)displayName version:(NSString *)version aboutDescription:(NSString *)aboutDescription hasWebContent:(BOOL)hasWebContent windowTitle:(NSString *)windowTitle bundleIdentifier:(NSString *)bundleIdentifier iconPath:(NSString *)iconPath windowLabel:(NSString *)windowLabel x:(double)x y:(double)y width:(double)width height:(double)height restoreFrame:(BOOL)restoreFrame resizable:(BOOL)resizable titlebarStyle:(int)titlebarStyle showPolicy:(int)showPolicy {
    ...
    // Open-document AppleEvents (Finder double-click, Open With,
    // `open -a App file`) route through the app delegate into the
    // runtime as OPEN_FILES events; without a delegate the paths die
    // with the event.
    self.appDelegate = [[NativeSdkAppDelegate alloc] init];
    self.appDelegate.host = self;
...
void native_sdk_appkit_emit_open_files(native_sdk_appkit_host_t *host, const char *const *paths, const size_t *path_lengths, size_t count) {
    NativeSdkAppKitHost *object = (__bridge NativeSdkAppKitHost *)host;
    NSMutableArray<NSString *> *filePaths = [NSMutableArray array];
    for (size_t index = 0; index < count; index++) {
        if (!paths[index] || path_lengths[index] == 0) continue;
        NSString *path = [[NSString alloc] initWithBytes:paths[index] length:path_lengths[index] encoding:NSUTF8StringEncoding];
        if (path.length > 0) [filePaths addObject:path];
    }
    if (filePaths.count == 0) return;
    [object.appDelegate emitPaths:filePaths];
}
