//! MDitor's markdown renderer: the Native SDK's GFM-subset mapper
//! (vendored from `@native-sdk/cli` 0.4.0 `src/primitives/canvas/markdown.zig`,
//! MIT) extended with LaTeX math and Mermaid diagram support.
//!
...
//! engine.
//!
//! MDitor extensions on top of the vendored subset:
//! - `$...$` inline math renders as a Unicode-transliterated span (see
//!   `math.zig`); `$$...$$` blocks render as composed math widgets.
//! - ` ```mermaid ` fences render as diagrams (flowchart/graph,
//!   sequenceDiagram, pie — see `mermaid.zig`); other languages keep the
//!   plain fenced-code panel.
//!
...
const text_spans = canvas.text_spans;
const ui_builder = canvas.ui_builder;
const math_renderer = @import("math.zig");
...
/// Math block source lines (the join buffer is fixed).
pub const max_math_block_lines: usize = 64;
    ...
    return struct {
        ...
        const Builder = struct {
            ...
            fn parseBlock(self: *Builder, lines: *LineIterator) ?Node {
                ...
                if (isMathBlockStart(trimmed)) return self.parseMathBlock(lines);
            ...
            /// `$$...$$` display math: a `$$` line opens the block, which
            /// closes on a line whose trimmed form is `$$` (multiline) or
            /// on the same line when the opening line already carries
            /// trailing content ending in `$$` (single-line form). An
            /// unclosed block degrades to the paragraph path (the `$$`
            /// stays literal) instead of swallowing the rest of the
            /// document.
            fn parseMathBlock(self: *Builder, lines: *LineIterator) ?Node {
                const open_line = lines.next() orelse return null;
                const open_trimmed = std.mem.trim(u8, open_line, " \t");
                var content: []const u8 = open_trimmed[2..];
                if (content.len >= 2 and std.mem.endsWith(u8, content, "$$")) {
                    content = std.mem.trim(u8, content[0 .. content.len - 2], " \t");
                } else if (std.mem.trim(u8, content, " \t").len == 0) {
                    // Multiline: consume lines until a closing `$$` — a
                    // line that starts with `$$`, or the line that ends
                    // the block (`$$...$$` on one line, content first).
                    // The content joins into ONE arena allocation
                    // (measure first, then copy: no realloc churn).
                    const start_index = lines.index;
                    var closed = false;
                    var pieces: [max_math_block_lines][]const u8 = undefined;
                    ...
                    while (lines.next()) |line| {
                        ...
                        if (piece_count >= pieces.len) break;
                        const piece = if (std.mem.endsWith(u8, trimmed, "$$") and trimmed.len > 2)
                            trimmed[0 .. trimmed.len - 2]
                        else
                            trimmed;
                        pieces[piece_count] = piece;
                        piece_count += 1;
                        total += piece.len + (if (piece_count > 1) @as(usize, 1) else 0);
                        if (std.mem.endsWith(u8, trimmed, "$$") and trimmed.len > 2) {
                            closed = true;
                            break;
                        }
                    }
                    if (!closed) {
                        // No closer: rewind so the paragraph path renders
                        // the opening `$$` as literal text.
                        lines.index = start_index;
                        return null;
                    }
                    const joined = self.ui.arena.alloc(u8, total) catch {
                        self.ui.failed = true;
                        return null;
                    };
                    var len: usize = 0;
                    for (pieces[0..piece_count]) |piece| {
                        ...
                        @memcpy(joined[len..][0..piece.len], piece);
                        len += piece.len;
                    }
                    content = joined[0..len];
                } else {
                    // Opening line carries content but no closing `$$`:
                    // treat the whole line as an unclosed inline attempt
                    // (literal).
                    return null;
                }
                content = std.mem.trim(u8, content, " \t");
                if (content.len == 0) return null;
                return math_renderer.Math(Msg).displayBlock(self.ui, content);
            }
            ...
            fn parseCodeFence(self: *Builder, lines: *LineIterator) ?Node {
                const open_line = lines.next() orelse return null;
                const info = std.mem.trim(u8, open_line[3..], " \t");
                ...
                // Mermaid fences render as diagrams when the renderer
                // knows the diagram type; unknown types and unparseable
                // sources fall back to the code panel below (with a
                // muted note).
                if (std.mem.eql(u8, info, "mermaid")) {
                    return mermaid_renderer.Mermaid(Msg).view(self.ui, code);
                }
            ...
            fn parseInline(self: *Builder, text: []const u8, base: TextSpan, spans: *[text_spans.max_text_spans_per_paragraph]TextSpan) []const TextSpan {
                ...
                while (index < text.len) {
                    ...
                    if (rest[0] == '`') {
                        ...
                        // `$...$` inline math: a single `$` (never the
                        // `$$` display opener) with a closer on the same
                        // line. The content transliterates to Unicode
                        // through the math vocabulary (see math.zig).
                        if (rest.len > 1 and rest[1] != '$') {
                            if (std.mem.indexOfScalarPos(u8, text, index + 1, '$')) |close| {
                                const content = text[index + 1 .. close];
                                if (content.len > 0 and std.mem.indexOfScalar(u8, content, '\n') == null and
                                    !std.mem.startsWith(u8, content, " ") and !std.mem.endsWith(u8, content, " "))
                                {
                                    flushLiteral(spans, &len, text[literal_start..index], base, bold, italic, strike);
                                    const buffer = self.ui.arena.alloc(u8, content.len * 3 + 64) catch {
                                        self.ui.failed = true;
                                        return spans[0..len];
                                    };
                                    const rendered = math_renderer.inlineText(content, buffer);
                                    appendSpan(spans, &len, spanWith(base, .{ .text = rendered, .monospace = true }));
                                    index = close + 1;
                                    literal_start = index;
                                    continue;
                                }
...
/// A display-math block opens on a line whose trimmed form starts with
/// `$$` (never `$$$`, which is a stray inline opener).
fn isMathBlockStart(line: []const u8) bool {
