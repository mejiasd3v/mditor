...
/// so 24 KiB keeps the two retained copies plus chrome text and span
/// payloads comfortably inside the budget (48 KiB used, 16 KiB spare).
pub const max_document_bytes = 24 * 1024;
...
const shell_views = [_]native_sdk.ShellView{
    .{ .label = canvas_label, .kind = .gpu_surface, .fill = true, .role = "MDitor viewer canvas", .accessibility_label = "MDitor", .gpu_backend = .metal, .gpu_pixel_format = .bgra8_unorm, .gpu_present_mode = .timer, .gpu_alpha_mode = .@"opaque", .gpu_color_space = .srgb, .gpu_vsync = true },
};
...
pub const Model = struct {
    ...
    /// Set when a file path arrived on the command line (Finder "Open
    /// With", `open -a`, or a terminal invocation): `boot` opens it
    /// before the first paint instead of the welcome sample.
    ...
    pub const RecentDoc = struct {
        ...
        name: []const u8,
        path: []const u8,
        /// Whether this entry is the currently open document (sidebar highlight).
        selected: bool,
    };
    ...
    pub fn recentDocs(model: *const Model, arena: std.mem.Allocator) []const RecentDoc {
        ...
        for (out, 0..) |*slot, index| {
            ...
            slot.* = .{
                .index = index,
                .name = pathBasename(entry),
                .path = entry,
                .selected = std.mem.eql(u8, entry, model.currentPath()),
            };
        }
        return out;
    }
...
/// The first non-flag command-line argument: the document path macOS
/// passes when the app is launched to open a file (Finder double-click,
/// "Open With", `open -a App file.md`). Single-document app: anything
/// after the first path is ignored. The result is copied into `buffer`
/// (the iterator's storage is transient).
pub fn firstFileArg(args: *std.process.Args.Iterator, buffer: []u8) ?[]const u8 {
    _ = args.skip(); // argv[0]: the executable
    while (args.next()) |arg| {
        if (arg.len == 0 or arg[0] == '-') continue;
        const len = @min(arg.len, buffer.len);
        @memcpy(buffer[0..len], arg[0..len]);
        return buffer[0..len];
    }
    return null;
}
...
pub const Msg = union(enum) {
    ...
    save_doc,
    save_as,
    /// A document macOS delivered via the open-document AppleEvent
    /// (Finder double-click, Open With, `open -a App file`) — the path
    /// slice is transient platform-event memory; `openPath` copies it
    /// immediately.
    open_file: []const u8,
...
/// recent list before the first paint.
pub fn boot(model: *Model, fx: *Effects) void {
    if (model.launch_pending) {
        model.launch_pending = false;
        openPath(model, fx, model.pendingPath());
    }
...
/// The open-document AppleEvent (Finder double-click, "Open With",
/// `open -a App file`) maps into an `open_file` Msg for the first path:
/// single-document app, extra files are ignored.
pub fn onOpenFiles(paths: []const []const u8) ?Msg {
    if (paths.len == 0) return null;
    return .{ .open_file = paths[0] };
}
...
pub fn update(model: *Model, msg: Msg, fx: *Effects) void {
    switch (msg) {
        ...
        .open_file => |path| openPath(model, fx, path),
...
pub fn main(init: std.process.Init) !void {
    ...
    // A file passed on the command line (Finder "Open With", `open -a`,
    // terminal invocation) opens at boot; without one the welcome sample
    // shows.
    var arg_buffer: [max_path_bytes]u8 = undefined;
    var args_iterator = std.process.Args.Iterator.init(init.minimal.args);
    if (firstFileArg(&args_iterator, &arg_buffer)) |launch_path| {
        model.setPendingPath(launch_path);
        model.launch_pending = true;
    }
    // Resolve where the recent list persists: the per-app data directory
    // (~/Library/Application Support/mditor on macOS). Failure
    ...
    if (app_dirs.resolveOne(.{ .name = "mditor" }, platform_value, env, .data, &dir_buffer)) |data_dir| {
