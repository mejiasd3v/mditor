...
const Harness = struct {
    ...
    fn snapshot(self: *Harness) native_sdk.automation.snapshot.Input {
        return self.harness.runtime.automationSnapshot("MDitor");
    }
...
test "the initial tree renders the welcome sample in editor and preview" {
    ...
    try testing.expect(std.mem.startsWith(u8, editor.text, "# MDitor"));
    try testing.expect(findByText(tree.root, .data_cell, "Open") != null);
    try testing.expect(subtreeHasText(tree.root, "MDitor"));
...
test "textarea compound edits keep the controlled document mirror synchronized" {
    var h = try Harness.create();
    defer h.destroy();

    // Start from a short source-driven value, then focus the real retained
    // textarea and type a multi-character committed-text event. Cmd+A is
    // the compound select+replace path (the runtime applies it to the
    // retained text first); every intermediate edit must still flow
    // through on-input so the model and runtime never diverge.
    ...
    // Cmd+A selects everything; typing replaces the selection.
    try h.harness.runtime.dispatchPlatformEvent(h.app, .{ .gpu_surface_input = .{
        .label = "viewer-canvas",
        .kind = .key_down,
        .key = "a",
    ...
    try testing.expectEqualStrings("Q", h.app_state.model.document());
    var retained = try h.harness.runtime.canvasWidgetLayout(1, "viewer-canvas");
    try testing.expectEqualStrings("Q", retained.findById(editor.id).?.widget.text);
    try testing.expectEqualDeep(canvas.TextSelection.collapsed(1), h.app_state.model.editor.selection);

    // Backspace deletes through the same path.
    try h.harness.runtime.dispatchPlatformEvent(h.app, .{ .gpu_surface_input = .{
        .label = "viewer-canvas",
        .kind = .key_down,
        .key = "backspace",
    } });
    try testing.expectEqualStrings("", h.app_state.model.document());
    retained = try h.harness.runtime.canvasWidgetLayout(1, "viewer-canvas");
    try testing.expectEqualStrings("", retained.findById(editor.id).?.widget.text);
    try testing.expectEqualDeep(canvas.TextSelection.collapsed(0), h.app_state.model.editor.selection);
}

test "textarea home/end moves keep the controlled selection synchronized" {
    var h = try Harness.create();
    defer h.destroy();
    ...
    // Home moves the caret to the start of the document, End to the end;
    // the retained widget and the model mirror must agree at every step.
    try h.harness.runtime.dispatchPlatformEvent(h.app, .{ .gpu_surface_input = .{
        .label = "viewer-canvas",
        .kind = .key_down,
        .key = "home",
    ...
    try testing.expectEqualDeep(canvas.TextSelection.collapsed(13), h.app_state.model.editor.selection);
    retained = try h.harness.runtime.canvasWidgetLayout(1, "viewer-canvas");
    try testing.expectEqualDeep(h.app_state.model.editor.selection, retained.findById(editor.id).?.widget.text_selection.?);
}

test "open_file from the open-document AppleEvent opens the first path" {
    var h = try Harness.create();
    defer h.destroy();
    const fx = &h.app_state.effects;

    // The event mapping picks the first path (single-document app) and
    // the Msg routes into the same open flow as the toolbar Open.
    const mapped = main.onOpenFiles(&.{ "/tmp/zn-ae-one.md", "/tmp/zn-ae-two.md" }).?;
    try testing.expectEqualStrings("/tmp/zn-ae-one.md", mapped.open_file);
    try h.dispatch(mapped);
    ...
    try testing.expectEqualStrings("/tmp/zn-ae-one.md", read_request.path);

    // No paths maps to no message.
    try testing.expect(main.onOpenFiles(&.{}) == null);
}
...
test "open and save round-trip through the fake executor" {
    ...
    model.setRecentStorePath("/tmp/zn-mditor-test/recent.txt");
    ...
    try testing.expectEqualStrings("/tmp/zn-mditor-test/recent.txt", recent_write.path);
...
// /tmp/icon-batch-shots/mditor-*-artifacts/. To use:
//
...
test "render icon-batch screenshots (env-gated)" {
    ...
    h.harness.runtime.options.automation = native_sdk.automation.Server.init(io, "/tmp/icon-batch-shots/mditor-light-artifacts", "MDitor");
    ...
    h.harness.runtime.options.automation = native_sdk.automation.Server.init(io, "/tmp/icon-batch-shots/mditor-dark-artifacts", "MDitor");
    try h.harness.runtime.dispatchAutomationCommand(h.app, "screenshot viewer-canvas 2");
}
...
// in /tmp/homepage-shots/mditor-{light,dark}-artifacts/. To use:
//
...
test "render homepage screenshots (env-gated)" {
    ...
    try h.harness.runtime.dispatchPlatformEvent(h.app, .{ .appearance_changed = .{ .color_scheme = .light } });
    h.harness.runtime.options.automation = native_sdk.automation.Server.init(io, "/tmp/homepage-shots/mditor-light-artifacts", "MDitor");
    try h.harness.runtime.dispatchAutomationCommand(h.app, "screenshot viewer-canvas 2");

    try h.harness.runtime.dispatchPlatformEvent(h.app, .{ .appearance_changed = .{ .color_scheme = .dark } });
    h.harness.runtime.options.automation = native_sdk.automation.Server.init(io, "/tmp/homepage-shots/mditor-dark-artifacts", "MDitor");
    try h.harness.runtime.dispatchAutomationCommand(h.app, "screenshot viewer-canvas 2");
}
...
test "firstFileArg picks the first non-flag command-line argument" {
    // macOS passes a document path as the first non-flag argv entry when
    // the app is launched to open a file (Finder double-click, Open With,
    // `open -a`). Flags and the executable itself must be skipped.
    const fake: std.process.Args = .{ .vector = &.{
        "mditor", "-psn_0_12345", "/tmp/note.md", "/tmp/other.md",
    } };
    var iterator = std.process.Args.Iterator.init(fake);
    var buffer: [main.max_path_bytes]u8 = undefined;
    const picked = main.firstFileArg(&iterator, &buffer).?;
    try testing.expectEqualStrings("/tmp/note.md", picked);

    // No file: null (the welcome sample shows instead).
    const bare: std.process.Args = .{ .vector = &.{"mditor"} };
    var bare_iterator = std.process.Args.Iterator.init(bare);
    try testing.expect(main.firstFileArg(&bare_iterator, &buffer) == null);

    // Over-capacity paths truncate into the buffer instead of overflowing.
    const long_path = "/tmp/" ++ "a" ** (main.max_path_bytes + 64);
    const long: std.process.Args = .{ .vector = &.{ "mditor", long_path } };
