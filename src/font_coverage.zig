...
//!
//! A codepoint outside `MDitorMathMono-Regular.ttf`'s character map
//! renders as a tofu box everywhere the bundled outlines are the only
//! glyph source —
...
/// Vendored from the Native SDK (MIT) and pointed at MDitor's bundled
/// math-mono face: the app's mono font (a JuliaMono subset, OFL) is
/// what math spans and code fences draw with, so coverage answers are
/// about THAT face — the tests pin every glyph the math/mermaid
/// vocabularies emit against it.
pub const face_name = "MDitor Math Mono";

const font_bytes: []const u8 = @embedFile("fonts/MDitorMathMono-Regular.ttf");
...
test "coverage answers for known covered and uncovered codepoints" {
    // The app's math-mono face (JuliaMono subset): ASCII, typographic
    // punctuation, Latin-1 letters, arrows, and the math vocabulary's
    // Greek/script/symbol glyphs all covered.
    for ([_]u21{ 'A', 'z', '0', ' ', 0x2026, 0x00B7, 0x2014, 0x00B1, 0x00F7, 0x2192, 0x00E9, 0x201C, 0x2019, 0x2022, 0x03B1, 0x03C0, 0x2211, 0x221A, 0x00B2, 0x2081, 0x2044, 0x2193, 0x25BC, 0x25B6, 0x21C4, 0x2264, 0x2260, 0x221E, 0x2208, 0x2113, 0x2318, 0x25D1 }) |cp| {
        try std.testing.expect(covers(cp));
    }
    // The recurring tofu class: ⑂, ✓, CJK, astral plane (JuliaMono
    // happens to draw ⌘ and ◑, which the stock Geist face did not).
    for ([_]u21{ 0x2442, 0x2713, 0x4E2D, 0x1F600 }) |cp| {
        try std.testing.expect(!covers(cp));
    }
}
